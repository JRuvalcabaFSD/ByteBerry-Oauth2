 ...
 *
 * Extends all properties from `SessionData` except for `createdAt` and `expiresAt`,
 * which are redefined here as optional `Date` fields. Also includes an optional
 * `ttlSeconds` property to specify the session's time-to-live in seconds.
 *
 * @property {number} [ttlSeconds] - Optional. The time-to-live for the session in seconds.
 * @property {Date} [createdAt] - Optional. The date and time when the session was created.
 * @property {Date} [expiresAt] - Optional. The date and time when the session will expire.
 */

export interface SessionProps extends Omit<SessionData, 'createdAt' | 'expiresAt'> {
	ttlSeconds?: number;
	createdAt?: Date;
	expiresAt?: Date;
}
...
export class SessionEntity {
	...
	public static create(props: SessionProps): SessionEntity {
		...
		return new SessionEntity({
			...
			expiresAt: props.expiresAt ?? new Date(now.getTime() + ttl * 1000),
